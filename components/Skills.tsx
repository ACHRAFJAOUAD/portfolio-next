...
export default function Skills() {
  return (
    <section id="skills" className="py-20 min-h-full">
      <div className="max-w-4xl mx-auto px-6 md:px-16">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {portfolioData.skills.map((skill, i) => (
            <div
              key={skill.id}
              className="bg-[#252526] border border-[#3c3c3c] rounded-xl hover:border-[#569cd6] transition-colors shadow-lg shadow-black/40" style={{padding: '2rem'}}
            >
              ...
              <div className="flex flex-wrap gap-3">
                {skill.stack.map((s) => (
                  <span
                    key={s}
                    className="font-mono text-xs px-3 py-1.5 bg-[#1e1e1e] text-[#ce9178] border border-[#3c3c3c] rounded-md"
                  >
                    &quot;{s}&quot;
                  </span>
                ))}
